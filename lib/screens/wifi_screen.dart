...
class _WifiScreenState extends ConsumerState<WifiScreen> {
  ...
  @override void initState() {
    super.initState();
    // Listen for router status changes OUTSIDE of build() to prevent flicker.
    // Using addPostFrameCallback so we have access to `ref` safely.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.listenManual(routerStatusProvider, (_, s) {
        if (!mounted) return;
        _syncFromStatus(s);
  ...
  // Called from a listener (not from build) so user edits never get clobbered.
  void _syncFromStatus(RouterStatus s) {
    bool changed = false;
    ...
    if (fp24.isNotEmpty && fp24 != '||' && fp24 != _lastSynced24) {
      ...
      // Channel width from chanspec — nctrlsb is the real sideband
  ...
  @override Widget build(BuildContext context) {
    ...
    // Watch only to trigger rebuild when wifi5present changes — sync handled by listener
