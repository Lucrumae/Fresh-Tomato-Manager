...
class _FilesScreenState extends ConsumerState<FilesScreen> {
  ...
  // ── Download: router POSTs file to Android HTTP server ──────────────────────
  Future<void> _download(_FsEntry entry) async {
    ...
    try {
      ...
      // Start HTTP server — router will POST file bytes here
      server          = await HttpServer.bind(InternetAddress.anyIPv4, 0);
      final port      = server.port;
      final ip        = await _localIp();
      if (ip == null) throw Exception('No local IP');

      // Set up receiver before telling router to send
      ...
      await ssh.runLong(
        "curl -s -X POST --data-binary @'$escapedSrc' http://$ip:$port/ 2>/dev/null"
        " || wget -O /dev/null --post-file='$escapedSrc' http://$ip:$port/ 2>/dev/null",
        timeout: const Duration(minutes: 10));

      final receivedBytes = await completer.future
          .timeout(const Duration(minutes: 10), onTimeout: () {
            server?.close(force: true);
            throw Exception('Download timed out');
          });

      if (receivedBytes.isEmpty) throw Exception('Received empty file');
      await File(localPath).writeAsBytes(receivedBytes);
  ...
  Future<void> _upload() async {
    ...
    try {
      ...
      // Start HTTP server — router wget will download from here
      server     = await HttpServer.bind(InternetAddress.anyIPv4, 0);
      final port = server.port;
      final ip   = await _localIp();
      if (ip == null) throw Exception('No local IP');
      ...
      // Use Completer so we know when router has finished downloading
      ...
      final dlErr = await ssh.runLong(
        "wget -O '$escapedPath' http://$ip:$port/ 2>&1",
        timeout: const Duration(minutes: 10));
      if (dlErr.toLowerCase().contains('error') || dlErr.toLowerCase().contains('failed')) {
        throw Exception('Router wget failed: $dlErr');
      }

      // Wait for server to confirm file was served
