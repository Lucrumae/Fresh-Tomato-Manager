...
class _FilesScreenState extends ConsumerState<FilesScreen> {
  ...
  Future<void> _download(_FsEntry entry) async {
    ...
    try {
      ...
      // Encode file as base64 on router using openssl, decode on phone
      // This avoids binary streaming issues with dartssh2 execute()
      final escapedSrc = entry.path.replaceAll("'", "'\\''");
      final session = await ssh.client!.execute(
        "openssl base64 -e -in '$escapedSrc' 2>/dev/null || "
        "{ cat '$escapedSrc' | openssl enc -base64 -A; echo; }"
      );

      session.stderr.drain<void>().catchError((_) {});

      final b64buf = StringBuffer();
      await for (final chunk in session.stdout) {
        if (chunk == null || chunk.isEmpty) continue;
        final s = chunk is Uint8List
          ? String.fromCharCodes(chunk)
          : chunk.toString();
        b64buf.write(s);
        setState(() => transfer.progress = -1);
      }
      await session.done.timeout(const Duration(seconds: 30), onTimeout: () {});

      // Decode base64 → binary
      final b64str = b64buf.toString().replaceAll(RegExp(r'\s'), '');
      if (b64str.isEmpty) {
        await raf.close();
        await outFile.delete();
        throw Exception('File is empty or cannot be read');
      }
      final decoded = base64Decode(b64str);
      await raf.writeFrom(decoded);
  ...
  Future<void> _upload() async {
    ...
    try {
      ...
      // Encode file as base64 on phone, decode with openssl on router
      // More reliable than binary stdin streaming with dropbear
      setState(() => transfer.progress = 0.1);
      final fileBytes = await localFile.readAsBytes();
      final b64 = base64Encode(fileBytes);
      setState(() => transfer.progress = 0.3);

      final session = await ssh.client!.execute(
        "openssl base64 -d -out '$escapedPath'"
      );
      session.stderr.drain<void>().catchError((_) {});

      // Add newlines every 64 chars (openssl base64 -d requires standard PEM format)
      final b64lines = StringBuffer();
      for (int i = 0; i < b64.length; i += 64) {
        b64lines.write(b64.substring(i, (i + 64).clamp(0, b64.length)));
        b64lines.write('\n');
      }
      final b64formatted = b64lines.toString();

      // Send in chunks
      const chunkSize = 4096;
      for (int i = 0; i < b64formatted.length; i += chunkSize) {
        final end = (i + chunkSize).clamp(0, b64formatted.length);
        session.stdin.add(Uint8List.fromList(b64formatted.substring(i, end).codeUnits));
        setState(() => transfer.progress = (0.3 + 0.7 * end / b64formatted.length).clamp(0.0, 0.99));
        await Future.delayed(Duration.zero);
      }
      await session.stdin.close();
      await session.done.timeout(const Duration(seconds: 60), onTimeout: () {});
  ...
  /// Menampilkan dialog penjelasan, lalu trigger popup permission OS.
  /// Hanya tampilkan in-app dialog jika PERMANENTLY denied.
  Future<bool> _ensureStoragePermission() async {
    final sdkInt = await _androidSdk();

    // Android 13+ → butuh MANAGE_EXTERNAL_STORAGE untuk tulis ke Download
    // Android 10-12 → READ/WRITE_EXTERNAL_STORAGE
    // Android 9-  → WRITE_EXTERNAL_STORAGE cukup
    ...
    // Tampilkan dialog penjelasan dulu sebelum popup OS
    if (status.isDenied && mounted) {
      final proceed = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          title: Row(children: [
            Icon(Icons.folder_rounded, color: _accent, size: 22),
            const SizedBox(width: 10),
            const Text('Izin Penyimpanan', style: TextStyle(fontSize: 16)),
          ]),
          content: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text(
              'Tomato Manager membutuhkan akses penyimpanan untuk menyimpan file yang di-download ke:',
            ...
            Container(
              ...
              child: Row(children: [
                Icon(Icons.save_alt_rounded, color: _accent, size: 16),
                const SizedBox(width: 8),
                Expanded(child: Text(
                  '/Download/Tomato Manager/',
                  style: TextStyle(
                    fontFamily: 'monospace', fontSize: 12, color: _accent),
                )),
              ]),
            ),
            const SizedBox(height: 10),
            const Text(
              'Izin ini diperlukan agar file bisa diakses dari File Manager ponsel Anda.',
    ...
    // Trigger popup permission sistem OS
    status = await perm.request();
    if (status.isGranted) return true;

    // Jika permanently denied → arahkan ke Pengaturan Aplikasi
      ...
      await showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          title: Row(children: [
            ...
            const SizedBox(width: 10),
            const Text('Izin Diblokir', style: TextStyle(fontSize: 16)),
          ]),
          content: const Text(
            'Izin penyimpanan diblokir secara permanen.\n\n'
            'Buka Pengaturan Aplikasi → Izin → Penyimpanan, lalu aktifkan secara manual.',
            style: TextStyle(fontSize: 13, height: 1.5),
          ),
          actions: [
            TextButton(
