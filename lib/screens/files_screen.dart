...
class _FilesScreenState extends ConsumerState<FilesScreen> {
  ...
  // ── Download: Android serves HTTP, router curl downloads ─────────────────────
  Future<void> _download(_FsEntry entry) async {
    ...
    try {
      ...
      // Spin up a one-shot HTTP server — router curl/wget POSTs the file to us
      ...
      // Tell router to POST the file to Android; receive it here
      final serverFuture = server.first.then((req) async {
      ...
      await ssh.run(
        ...
        "|| wget -q -O /dev/null --post-file='\$escapedSrc' http://\$ip:\$port/ 2>/dev/null"
      );

      final receivedBytes = await serverFuture
          .timeout(const Duration(seconds: 60), onTimeout: () => throw Exception('Download timed out'));
  ...
  Future<void> _upload() async {
    ...
    try {
      ...
      // Serve file over HTTP — router wget downloads it directly, binary perfect
      ...
      final dlErr = await ssh.run(
        "wget -q -O '\$escapedPath' http://\$ip:\$port/ 2>&1"
      );
