...
class _FilesScreenState extends ConsumerState<FilesScreen> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('✓ Saved to: $localPath'),
  ...
  Future<void> _showContent(_FsEntry entry) async {
    if (entry.size > 512 * 1024) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('File too large (>512KB)')));
      return;
    }
  ...
  /// Shows explanation dialog, then triggers OS permission popup.
  /// Only shows in-app settings redirect if PERMANENTLY denied.
  Future<bool> _ensureStoragePermission() async {
    ...
    // Show rationale dialog before triggering OS popup
      ...
      final proceed = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text(
              'Tomato Manager needs storage access to save downloaded files to:',
            ...
            const SizedBox(height: 10),
            const Text(
              'This allows files to be accessed directly from your phone\'s File Manager.',
    ...
    // Trigger OS system permission popup
      ...
      await showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          title: Row(children: [
            ...
            const SizedBox(width: 10),
            const Text('Permission Blocked', style: TextStyle(fontSize: 16)),
          ]),
          content: const Text(
