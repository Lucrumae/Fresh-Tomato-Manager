...
final qosClassifyProvider = FutureProvider.autoDispose<List<Map<String, String>>>((ref) async {
  ref.watch(_classifyTickProvider);
  final ssh = ref.read(sshServiceProvider);
  if (!ssh.isConnected) return [];
  try {
    ...
    const protoMap = {'-1':'Any','0':'Any','6':'TCP','17':'UDP','256':'TCP/UDP','1':'ICMP'};
    const portTypeMap = {'a':'Any','d':'Dst','s':'Src','b':'Both'};

    // VERIFIED FreshTomato qos_orules format from actual router debug:
    // "0<<-1<d<53<0<<0:10<<0<DNS"
    // Split by <: [0]=addr_type, [1]=addr(mac/ip), [2]=proto, [3]=port_type,
    //             [4]=port, [5]=ipp2p, [6]=layer7, [7]=pkt_size, [8]=empty,
    //             [9]=prio_idx (0-based), [10]=desc
    ...
    for (final chunk in rulesRaw.split('>')) {
      ...
      final addrType  = f[0].trim();                      // 0=any, else mac/ip
      final addr      = f[1].trim();                      // mac or ip address
      final protoRaw  = f[2].trim();
      final portType  = f[3].trim();
      final port      = f[4].trim();
      final ipp2p     = f[5].trim();
      final layer7    = f[6].trim();
      final pktRaw    = f[7].trim();                      // "start:end" KB range
      final prioRaw   = f[9].trim();                      // 0-indexed class index
      final desc      = f.length > 10 ? f[10].trim() : '';

      final proto     = protoMap[protoRaw] ?? (protoRaw.isEmpty ? 'Any' : protoRaw);
      final portLabel = portTypeMap[portType] ?? portType;

      // Parse packet size range
      String kb1 = '0', kb2 = '-1';
      if (pktRaw.contains(':')) {
        final p = pktRaw.split(':');
        kb1 = p[0].trim().isEmpty ? '0'  : p[0].trim();
        kb2 = p[1].trim().isEmpty ? '-1' : p[1].trim();
      }

      // prio_idx is 0-based → display as P1..P10
      final prioInt   = int.tryParse(prioRaw) ?? 0;
      final prioDisp  = (prioInt + 1).toString();         // 1-indexed for display
      final className = prioInt < classNames.length    ? classNames[prioInt]
                      : prioInt < defClassNames.length ? defClassNames[prioInt]
                      : 'P${prioInt + 1}';

      final portDisplay = port.isNotEmpty ? (portLabel != 'Any' ? '$portLabel: $port' : port) : 'Any';

      rules.add({
        'prio':        prioRaw,       // raw 0-indexed (for save)
        'prioDisp':    prioDisp,      // 1-indexed (for display)
        'className':   className,
        'addrType':    addrType,
        'addr':        addr,
        'proto':       proto,
        ...
        'kb1':         kb1,
        'kb2':         kb2,
        'desc':        desc.isNotEmpty ? desc : className,
        'rawDesc':     desc,
...
class BandwidthScreen extends ConsumerStatefulWidget {
  final bool initialShowQos;
  final bool hideAppBar;
  const BandwidthScreen({super.key, this.initialShowQos = false, this.hideAppBar = false});
  @override
...
class _BandwidthScreenState extends ConsumerState<BandwidthScreen> {
  ...
  void initState() {
    super.initState();
    _showQos = widget.initialShowQos;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: widget.hideAppBar ? null : AppBar(
        ...
        title: widget.hideAppBar ? const SizedBox.shrink() : Row(mainAxisSize: MainAxisSize.min, children: [
          GestureDetector(
  ...
  Future<void> _saveRules(List<Map<String, String>> rules) async {
    ...
    try {
      ...
      String encRule(Map<String, String> r) {
        // If rawChunk exists and rule was not modified, preserve exactly
        final raw = r['rawChunk'] ?? '';
        if (raw.isNotEmpty && r['_modified'] != '1') return raw;

        // VERIFIED format: addrType<<proto<portType<port<ipp2p<layer7<pktSize<<prioIdx<desc
        // e.g. "0<<17<d<5000:5500<0<<<0:2<<1<General Gaming"
        const p2n = {'Any':'-1','TCP':'6','UDP':'17','TCP/UDP':'256','ICMP':'1'};
        final addrType = r['addrType'] ?? '0';
        final addr     = r['addr'] ?? '';
        final proto    = r['proto'] ?? 'Any';
        final protoNum = p2n[proto] ?? '-1';
        final portType = r['portType']?.isNotEmpty == true ? r['portType']! : 'a';
        final port     = r['port'] ?? '';
        final ipp2p    = r['ipp2p'] ?? '0';
        final layer7   = r['layer7'] ?? '';
        final kb1      = r['kb1'] ?? '0';
        final kb2      = r['kb2'] ?? '-1';
        final prio     = r['prio'] ?? '0';   // 0-indexed class index
        final desc     = r['rawDesc']?.isNotEmpty == true ? r['rawDesc']! : (r['desc'] ?? '');
        // Format verified: [addrType]<[addr]<[proto]<[portType]<[port]<[ipp2p]<[layer7]<[pktSize]<[empty]<[prioIdx]<[desc]
        return '$addrType<$addr<$protoNum<$portType<$port<$ipp2p<$layer7<$kb1:$kb2<<$prio<$desc';
      }
            ...
            actions: [
              ...
              ElevatedButton(
                ...
                onPressed: () {
                  ...
                  // Port type for save: any=a, dst=d, src=s, srcordst=b
                  const pt2s = {'any':'a','dst':'d','src':'s','srcordst':'b'};
                  // Determine addrType: 0=any, mac=mac addr, ip=ip addr
                  final addrTypeNum = (addrType == 'any') ? '0' : addrCtrl.text.trim();
                  final addrMac     = addrType == 'mac' ? addrCtrl.text.trim() : '';
                  final portCombined = p1.isNotEmpty && p2.isNotEmpty && p1 != p2 ? '\$p1:\$p2' : p1;
                  final prioDispNew  = ((int.tryParse(prioIdx) ?? 0) + 1).toString();
                  final newRule = <String, String>{
                    'prio':      prioIdx,            // 0-indexed for save
                    'prioDisp':  prioDispNew,         // 1-indexed for display
                    'addrType':  addrType == 'any' ? '0' : '0',
                    'addr':      addrMac,
                    'proto':     proto,              // human (Any/TCP/UDP)
                    'protoRaw':  protoNum,
                    'portType':  pt2s[portType] ?? 'a',
                    'port':      portCombined,
                    'ipp2p':     ipp2p == 'disabled' ? '0' : ipp2p,
                    'layer7':    layer7 == 'disabled' ? '' : layer7,
                    'kb1':       kbFrom,
                    'kb2':       kbTo,
                    'rawDesc':   descCtrl.text.trim(),
                    'desc':      descCtrl.text.trim().isNotEmpty ? descCtrl.text.trim() : classNames[int.tryParse(prioIdx) ?? 0],
                    'className': classNames[(int.tryParse(prioIdx) ?? 0).clamp(0, 9)],
                    '_modified': '1',
                  };
          ...
          displayList.isEmpty
            ...
            : ReorderableListView.builder(
                ...
                itemBuilder: (_, i) {
                  ...
                  final prio     = r['prioDisp'] ?? r['prio'] ?? '1';
                  final prioKey  = prio; // 1-indexed for color
                  final color    = prioColors[prioKey] ?? accent;
