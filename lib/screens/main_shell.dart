...
class _MainShellState extends ConsumerState<MainShell>
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    switch (state) {
      ...
      case AppLifecycleState.paused:
        // App going to background — update notification so user can see status
        final cfg = ref.read(configProvider);
        NotificationService.showConnected(cfg?.host ?? 'router');
      default:
        break;
    }
  }
  ...
  Future<void> _boot() async {
    ...
    // Step 1: SSH — already connected by _AutoConnect in main.dart.
    // Use connectIfNeeded so we don't tear down a live session if somehow called again.
    ...
    // Step 2: Wake + start pollers
    if (mounted) setState(() { _bootStep = 'Loading system data…'; _bootProgress = 1; });
    await WakelockPlus.enable();
    _startPollers();

    // Step 3: Wait for first data from router
    if (mounted) setState(() { _bootStep = 'Fetching router status…'; _bootProgress = 2; });
    await _waitForInitialData();

    // Step 4: Connection keeper
    ...
    BackgroundService.onDisconnectAction = _onConnectionFailed;
    unawaited(BackgroundService.start(
      host:       cfg.host,
      statusText: 'Connected — ${cfg.host}',
    ));
    unawaited(NotificationService.showConnected(cfg.host));
  }
