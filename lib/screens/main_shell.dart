...
class _MainShellState extends ConsumerState<MainShell>
  ...
  Future<void> _boot() async {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (!mounted) return;
      // Start foreground service (includes wakelock internally)
      try {
        await BackgroundService.start(
          host: cfg.host, statusText: 'Connected to ${cfg.host}');
      } catch (_) {}
      // Show battery optimization popup after service is started
  ...
  Future<void> _checkBatteryOptimization() async {
    ...
    if (userAllowed == true) {
      ...
      // Note: after requestIgnoreBatteryOptimization(), Android may pause/resume the Activity.
      // The widget might be unmounted — do NOT do anything with context after this point.
    }
  }
