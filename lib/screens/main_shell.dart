...
class _MainShellState extends ConsumerState<MainShell>
  ...
  Future<void> _boot() async {
    ...
    // Start background keep-alive service and wakelock now that we're fully mounted.
    // Doing this here (not in SetupScreen) avoids context-after-dispose crashes.
    BackgroundService.start(
      host: cfg.host, statusText: 'Connected to ${cfg.host}').catchError((_) {});
    ...
    // After overview is visible, check battery optimization (first-run only)
  ...
  // ── Battery optimization popup (shown once on overview) ────────────────────
  Future<void> _checkBatteryOptimization() async {
    if (!Platform.isAndroid) return;
    bool exempt = false;
    try {
      exempt = await FlutterForegroundTask.isIgnoringBatteryOptimizations;
    } catch (_) {}
    ...
    final allowed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(children: [
          ...
          const Expanded(child: Text('Keep Connection Alive',
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Container(
            ...
            child: Column(children: [
              _BulletItem(icon: Icons.sync_rounded, color: v.accent,
                title: 'Connection stays active',
                desc: 'SSH remains alive while app is in background'),
              const SizedBox(height: 8),
              _BulletItem(icon: Icons.notifications_active_rounded, color: v.accent,
                title: 'Router status notification',
                desc: 'Persistent notification while connected'),
              const SizedBox(height: 8),
              _BulletItem(icon: Icons.battery_4_bar_rounded, color: Colors.orange,
...
// ── Bullet item for battery optimization dialog ───────────────────────────────
class _BulletItem extends StatelessWidget {
  final IconData icon;
  final Color color;
  final String title;
  final String desc;
  const _BulletItem({required this.icon, required this.color,
