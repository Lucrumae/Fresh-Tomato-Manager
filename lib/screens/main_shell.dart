...
class _MainShellState extends ConsumerState<MainShell>
  ...
  Future<void> _boot() async {
    ...
    // Start ConnectionKeeper
    final keeper = ref.read(connectionKeeperProvider);
    keeper.onFailed = _onConnectionFailed;
    keeper.start();

    // Show main UI immediately — don't block on permission dialog
    if (mounted) setState(() => _booting = false);

    // Start foreground service AFTER UI is shown (permission dialog appears on top)
    // This prevents the 2-3s delay from the OS notification permission dialog
