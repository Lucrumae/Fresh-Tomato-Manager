...
class _DevicesScreenState extends ConsumerState<DevicesScreen> {
  ...
  @override Widget build(BuildContext context) {
    ...
    final filtered = devs.where((d) {
      if (_filter=='wifi'    && (d.isWan || !d.isWireless)) return false;
      if (_filter=='eth'     && (!d.isWan && d.isWireless)) return false;
      ...
      body: Column(children:[
        ...
        SizedBox(height:36, child:ListView(
          ...
          children:[
            ...
            _Chip('WiFi',    'wifi',    devs.where((d)=>d.isWireless && !d.isWan).length,           _filter, ()=>setState(()=>_filter='wifi'),    v),
            _Chip('Ethernet','eth',     devs.where((d)=>!d.isWireless || d.isWan).length,           _filter, ()=>setState(()=>_filter='eth'),     v),
    ...
    return VCard(
      ...
      child:Column(crossAxisAlignment:CrossAxisAlignment.start, children:[
          ...
          Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start, children:[
            Text(dev.displayName,
              style:GoogleFonts.outfit(fontSize:13, fontWeight:FontWeight.w700,
                color: dev.isWan ? V.info : (dev.isBlocked?V.err:v.hi)),
              overflow:TextOverflow.ellipsis),
            Text(dev.isWan && dev.hostname.isNotEmpty ? dev.hostname : dev.ip,
              style:GoogleFonts.dmMono(fontSize:11, color:v.mid)),
          ])),
          // Block toggle — not applicable for WAN gateway
          if (!dev.isWan) _BlockButton(device:dev),
        ]),
        ...
        Row(children:[
          ...
          if (dev.isWan) ...[
            const SizedBox(width:6),
            _WanTag(dev.interface, v),
          ] else if (dev.isWireless && dev.wifiBand.isNotEmpty) ...[
            _BandTag(dev.wifiBand, v),
            if (dev.rssi.isNotEmpty) ...[
              const SizedBox(width:6),
              _Tag(dev.rssi, v),
            ],
          ],
...
Widget _WanTag(String iface, VC v) {
  const color = V.warn;
  final label = iface.isNotEmpty && iface != 'wan' ? 'WAN: $iface' : 'WAN';
  return Container(
    padding:const EdgeInsets.symmetric(horizontal:6, vertical:2),
    decoration:BoxDecoration(
    ...
    child:Row(mainAxisSize:MainAxisSize.min, children:[
      Icon(Icons.public_rounded, size:9, color:color),
      const SizedBox(width:3),
      Text(label, style:GoogleFonts.dmMono(fontSize:9, color:color, fontWeight:FontWeight.w600)),
    ]));
}
