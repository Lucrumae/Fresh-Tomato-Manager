...
class _BandwidthTab extends StatelessWidget {
  const _BandwidthTab();
  @override Widget build(BuildContext context) =>
    const BandwidthScreen(initialShowQos: false, hideAppBar: true);
}
...
class _QosTab extends StatelessWidget {
  const _QosTab();
  @override Widget build(BuildContext context) =>
    const BandwidthScreen(initialShowQos: true, hideAppBar: true);
}

// ── WiFi settings tab ─────────────────────────────────────────────────────────
class _WifiTab extends StatelessWidget {
  const _WifiTab();
  @override Widget build(BuildContext context) => const WifiScreen();
}
      ...
      child: Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            Text(':${rule.externalPort}${rule.internalPort != rule.externalPort ? "→${rule.internalPort}" : ""} → ${rule.internalIp}${rule.srcFilter.isNotEmpty ? " [src: ${rule.srcFilter}]" : ""}',
...
class _PfDialogState extends State<_PfDialog> {
  ...
  @override Widget build(BuildContext ctx) => AlertDialog(
    ...
    actions: [
      ...
      TextButton(
        onPressed: () {
          final extP = _ext.text.trim();
          final intP = _intP.text.trim();
          if (extP.isEmpty || _ip.text.isEmpty) return;
          Navigator.pop(ctx, PortForwardRule(
            id: DateTime.now().millisecondsSinceEpoch.toString(),
            name: _name.text.trim(), protocol: _proto,
            externalPort: extP,
            internalPort: intP.isEmpty ? extP : intP,
