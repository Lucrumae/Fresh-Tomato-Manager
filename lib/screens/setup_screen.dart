...
class _SetupScreenState extends ConsumerState<SetupScreen>
  ...
  Future<void> _connect() async {
    ...
    // Android: show battery optimization popup every connect until granted
    if (Platform.isAndroid) {
      // Check via flutter_foreground_task — more reliable than permission_handler
      bool batteryOptExempt = false;
      try {
        batteryOptExempt = await FlutterForegroundTask.isIgnoringBatteryOptimizations;
      } catch (_) {
        // Fallback to permission_handler if flutter_foreground_task check fails
        batteryOptExempt = await Permission.ignoreBatteryOptimizations.isGranted;
      }

      final needsRequest = !batteryOptExempt;
      if (needsRequest) {
        if (!mounted) return;

        // Step 1: Show in-app rationale dialog
        final userAllowed = await _showWakelockRationaleDialog();
        if (!mounted) return;

        if (userAllowed) {
          // Step 2: Enable wakelock (no context needed)
          await WakelockPlus.enable().catchError((_) {});

          // Step 3: Start background service — must happen before navigate
          // so the foreground service window is valid when system dialog appears
          await BackgroundService.start(
            host: host, statusText: 'Connected to $host').catchError((_) {});

          // Step 4: Trigger Android system battery optimization dialog.
          // flutter_foreground_task fires ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS
          // intent which is the correct way — permission_handler does NOT reliably
          // show this dialog on all Android versions.
          try {
        ...
        // Step 5: Navigate only after all permission dialogs are done
        if (!mounted) return;
      ...
      // Permission already granted — start services silently then navigate
      await WakelockPlus.enable().catchError((_) {});
      await BackgroundService.start(
        host: host, statusText: 'Connected to $host').catchError((_) {});
    }
  ...
  /// In-app rationale dialog explaining why battery optimization exemption is needed.
  /// Returns true if user taps Allow.
  Future<bool> _showWakelockRationaleDialog() async {
