...
class _SetupScreenState extends ConsumerState<SetupScreen>
  ...
  Future<void> _connect() async {
    ...
    // Show wakelock dialog on FIRST connect only (Android)
    bool showWakelockDialog = false;
    if (Platform.isAndroid) {
      final wakelockAsked = p.getBool('wakelock_asked') ?? false;
      if (!wakelockAsked) {
        // Mark asked NOW before showing dialog so it never shows twice
    ...
    if (showWakelockDialog) {
      // Show in-app rationale — capture result before navigating
      final granted = await _showWakelockRationaleDialog();
      await p.setBool('wakelock_enabled', granted);
      if (!mounted) return;
      // Navigate first, THEN start background services to avoid context issues
      ...
      // Returning user — restore wakelock preference silently
      final wakelockEnabled = p.getBool('wakelock_enabled') ?? false;
      if (Platform.isAndroid && wakelockEnabled) {
        await WakelockPlus.enable().catchError((_) {});
        await BackgroundService.start(
          host: host, statusText: 'Connected to $host').catchError((_) {});
      }
  ...
  /// In-app rationale dialog. Returns true if user taps Allow.
  /// Does NOT start any services — caller handles that after navigation.
  ...
  /// Triggers the real Android system popup:
