...
class _SetupScreenState extends ConsumerState<SetupScreen>
  ...
  Future<void> _connect() async {
    ...
    // Minta izin Wake Lock agar koneksi SSH tidak terputus di latar belakang
    if (Platform.isAndroid && mounted) {
      ...
      if (!wakelockAsked) {
        ...
        // Sudah pernah izin → aktifkan langsung
        final wakelockEnabled = p.getBool('wakelock_enabled') ?? true;
        if (wakelockEnabled) {
          await WakelockPlus.enable();
          await BackgroundService.start(host: host, statusText: 'Terhubung ke $host');
        }
      }
    }
  ...
  /// Menampilkan popup izin Wake Lock sistem ke pengguna
    ...
    final granted = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(children: [
          ...
          const Expanded(
            child: Text('Jaga Koneksi Aktif',
              style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
          ),
        ]),
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          const Text(
            'Izinkan Tomato Manager menjaga koneksi SSH tetap aktif saat berpindah aplikasi atau layar mati?',
          ...
          Container(
            ...
            child: Column(children: [
              _PermissionItem(
                ...
                color: v.accent,
                title: 'Koneksi tidak terputus',
                desc: 'SSH tetap aktif di latar belakang',
              ),
              const SizedBox(height: 8),
              _PermissionItem(
                icon: Icons.notifications_active_rounded,
                color: v.accent,
                title: 'Notifikasi status router',
                desc: 'Tampil saat app berjalan di background',
              ),
              const SizedBox(height: 8),
              _PermissionItem(
                icon: Icons.battery_4_bar_rounded,
                color: Colors.orange,
                title: 'Penggunaan baterai sedikit lebih tinggi',
                desc: 'Hanya aktif saat terhubung ke router',
              ),
            ]),
          ),
        ]),
        ...
        actions: [
          ...
          ElevatedButton.icon(
            ...
            label: const Text('Izinkan', style: TextStyle(fontWeight: FontWeight.w700)),
