...
class _SetupScreenState extends ConsumerState<SetupScreen>
  ...
  Future<void> _connect() async {
    ...
    // Step 1: Cek & minta izin battery optimization (Android only)
  ...
  // Cek battery optimization — tampilkan popup jika belum dikecualikan.
  // Return true = lanjut connect, false = batalkan.
  Future<bool> _ensurePermissions() async {
    final alreadyExempt = await Permission.ignoreBatteryOptimizations.isGranted;
        ...
        return AlertDialog(
          ...
          title: Row(children: [
            Container(
              ...
              child: Icon(Icons.battery_saver_rounded, color: v.accent, size: 22),
            ),
            const SizedBox(width: 12),
            const Expanded(
              child: Text('Izin Background',
                style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
            ),
          ]),
          content: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text(
              'Agar koneksi SSH tetap aktif saat layar mati atau berpindah aplikasi, '
              'Tomato Manager perlu dikecualikan dari optimasi baterai.',
            ...
            Container(
              ...
              child: Column(children: [
                _PermRow(Icons.wifi_tethering_rounded, v.accent,
                  'Koneksi tetap hidup', 'SSH tidak terputus saat layar mati'),
                const SizedBox(height: 8),
                _PermRow(Icons.battery_4_bar_rounded, Colors.orange,
                  'Sedikit lebih boros baterai', 'Hanya aktif selama terhubung ke router'),
              ]),
            ),
            ...
            Container(
              ...
              child: const Row(children: [
                ...
                SizedBox(width: 8),
                Expanded(child: Text(
                  'Dialog izin sistem Android akan muncul berikutnya.',
          ...
          actions: [
            TextButton(
              ...
              child: const Text('Lewati', style: TextStyle(color: Colors.white38, fontSize: 12)),
            ),
            ElevatedButton.icon(
              ...
              label: const Text('Izinkan', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13)),
    ...
    // Apapun pilihan user (Izinkan/Lewati), tetap lanjut connect
...
// ── Permission popup row item ──────────────────────────────────────────────
class _PermRow extends StatelessWidget {
  final IconData icon;
  final Color    color;
  final String   title, desc;
  const _PermRow(this.icon, this.color, this.title, this.desc);
  @override Widget build(BuildContext context) =>
