...
class _SetupScreenState extends ConsumerState<SetupScreen>
  ...
  Future<void> _connect() async {
    ...
    // Android: every connect, show popup until battery optimization is granted
    if (Platform.isAndroid) {
      final batteryStatus = await Permission.ignoreBatteryOptimizations.status;
      if (!batteryStatus.isGranted) {
        // Show in-app rationale first
        final userAllowed = await _showWakelockRationaleDialog();
        if (!mounted) return;
        // Navigate FIRST to avoid context-after-dispose crash
        ...
        if (userAllowed) {
          ...
          // Trigger the real Android system popup
          await Permission.ignoreBatteryOptimizations.request().catchError((_) {});
        }
        return;
      }
      // Permission already granted — start services silently
      await WakelockPlus.enable().catchError((_) {});
      await BackgroundService.start(
        host: host, statusText: 'Connected to $host').catchError((_) {});
    }
