...
class _RouterTabState extends ConsumerState<_RouterTab> {
  ...
  Future<void> _restore() async {
    ...
    try {
      ...
      // Step 3: Upload file to router /tmp/configuration.cfg
      final uploadErr = await ssh.uploadBytes(bytes, '/tmp/configuration.cfg');
      if (uploadErr != null) throw Exception('Upload failed: $uploadErr');

      // Verify decoded binary size matches local file size exactly
      final sizeOut    = await ssh.run('wc -c < /tmp/configuration.cfg');
      final remoteSize = int.tryParse(sizeOut.trim()) ?? 0;
      if (remoteSize == 0) throw Exception('Uploaded file is empty on router');
      if (remoteSize != bytes.length) {
        throw Exception('Size mismatch: local=${bytes.length}B remote=${remoteSize}B');
      }

      // Step 4: nvram restore + commit
