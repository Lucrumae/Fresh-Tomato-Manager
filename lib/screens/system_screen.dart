...
class _LogsTabState extends ConsumerState<_LogsTab> {
  ...
  bool  _atBottom = false; // tracks whether user is near bottom
  ...
  void _onScroll() {
    ...
    final nearBottom = _scroll.position.maxScrollExtent - _scroll.offset < 80;
  ...
  @override Widget build(BuildContext context) {
    ...
    // Format time string — show only HH:MM:SS if same year, or MMM DD HH:MM:SS
    String fmtTime(DateTime t) {
      final now = DateTime.now();
      final ts = t.toLocal();
      if (ts.year == now.year) {
        final mon = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][ts.month-1];
        return '$mon ${ts.day.toString().padLeft(2)} ${ts.hour.toString().padLeft(2,'0')}:${ts.minute.toString().padLeft(2,'0')}:${ts.second.toString().padLeft(2,'0')}';
      }
    ...
    return Stack(children: [
      Column(children: [
        ...
        // Log lines — scroll position preserved across polls
        Expanded(child: filtered.isEmpty
          ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.article_outlined, size: 32, color: v.lo),
              const SizedBox(height: 8),
              Text('No logs', style: GoogleFonts.dmMono(fontSize: 12, color: v.lo)),
            ]))
          : ListView.builder(
              controller: _scroll,
                ...
                return Padding(
                  ...
                  child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Container(width: 2, height: 14,
                      margin: const EdgeInsets.only(right: 7, top: 2), color: c),
                    Expanded(child: Text.rich(TextSpan(children: [
                      TextSpan(text: '${fmtTime(e.time)} ',
                        style: GoogleFonts.dmMono(fontSize: 9, color: v.lo)),
                      if (e.process.isNotEmpty)
                        TextSpan(text: '[${e.process}] ',
                          style: GoogleFonts.dmMono(fontSize: 9, color: v.mid)),
                      TextSpan(text: e.message,
                        style: GoogleFonts.dmMono(fontSize: 9, color: c)),
                    ]))),
                  ]),
                );
              },
      ...
      // "Scroll to latest" FAB — appears when user is NOT at bottom
      if (!_atBottom && filtered.isNotEmpty)
        Positioned(
          bottom: 16, right: 16,
          child: GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [BoxShadow(
                  color: v.accent.withOpacity(0.35), blurRadius: 12)]),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.arrow_downward_rounded, size: 13,
                  color: v.dark ? V.d0 : Colors.white),
                const SizedBox(width: 5),
                Text('Latest', style: GoogleFonts.outfit(
...
class _RouterTabState extends ConsumerState<_RouterTab> {
      ...
      children: [
        ...
        VCard(padding: const EdgeInsets.all(14), child: Column(children: [
          ...
          _Row(icon: Icons.upload_rounded,       color: V.info,
            title: 'Import Config',  sub: 'Restore from .cfg backup file',
  ...
  Future<void> _firmwareUpgrade(BuildContext ctx) async {
    ...
    HttpServer? server;
    SSHClient? flashClient;
    try {
      final cfg   = ref.read(configProvider)!;
      final ssh   = ref.read(sshServiceProvider);
      final bytes = await File(filePath).readAsBytes();

      // ── Step 1: Upload firmware via HTTP ────────────────────────────────────
      ...
      // Remote filename matches original extension (.trx or .bin)
      final remoteFw = name.endsWith('.bin') ? '/tmp/firmware.bin' : '/tmp/firmware.trx';

      // Use main session to wget — runLong 10 min
      final wgetOut = await ssh.runLong(
        'wget -O $remoteFw http://$ip:$port/',
        timeout: const Duration(minutes: 10));
      if (wgetOut.toLowerCase().contains('error')
          || wgetOut.toLowerCase().contains('failed')
          || wgetOut.toLowerCase().contains('refused')) {
        await ssh.run('rm -f $remoteFw');
        throw Exception('Download failed: $wgetOut');
      }

      // ── Step 2: Open a dedicated SSH session just for flashing ──────────────
      // The flash command reboots the router which kills the session.
      // Using a separate SSHClient avoids corrupting the main session queue.
      flashClient = SSHClient(
        await SSHSocket.connect(cfg.host, cfg.sshPort,
            timeout: const Duration(seconds: 15)),
        username: cfg.username,
        onPasswordRequest: () => cfg.password,
      );
      await flashClient.authenticated.timeout(const Duration(seconds: 15));

      // ── Step 3: Flash — single command, no fallback ──────────────────────────
      final session = await flashClient.execute(
        'mtd-write2 $remoteFw linux && nvram erase && reboot');
      session.stdin.close();

      // Collect output until session ends (router reboots = connection drops)
      ...
      // Show flash output (e.g. 'CRC OK - Image successfully flashed\nRebooting...')
      final flashMsg = out.toString().trim();
      final displayMsg = flashMsg.isNotEmpty
          ? 'Flashing… router will reboot\n$flashMsg'
          : 'Flashing… router will reboot';
      ...
      // Connection reset / closed by remote = router rebooted = success
      final msg = e.toString().toLowerCase();
      if (msg.contains('connection reset') || msg.contains('closed')
          || msg.contains('broken pipe') || msg.contains('socket')) {
        if (ctx.mounted) ScaffoldMessenger.of(ctx).showSnackBar(
          const SnackBar(content: Text('Flashing… router will reboot'),
              backgroundColor: V.ok, duration: Duration(seconds: 8)));
      } else {
        if (ctx.mounted) ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(
      ...
      flashClient?.close();
      await server?.close(force: true);
      if (mounted) setState(() => _fwBusy = false);
    }
  }
  ...
  Future<void> _backup() async {
    ...
    // Minta izin storage sebelum mulai backup
    ...
    try {
      await ssh.run('nvram save /tmp/nvram_backup.cfg 2>/dev/null');
      ...
      await ssh.runLong(
        ...
        'wget -O /dev/null --post-file=/tmp/nvram_backup.cfg http://$ip:$port/ 2>/dev/null',
        timeout: const Duration(minutes: 5));
      final bytes = await comp.future.timeout(const Duration(minutes: 5));
  ...
  Future<bool> _ensureStoragePermission() async {
    ...
    // Show rationale dialog before OS popup
    if (status.isDenied && mounted) {
      ...
      final proceed = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          title: Row(children: [
            Icon(Icons.save_alt_rounded, color: v.accent, size: 22),
            const SizedBox(width: 10),
            const Text('Storage Permission', style: TextStyle(fontSize: 16)),
          ]),
          content: Column(mainAxisSize: MainAxisSize.min, children: [
            const Text(
              'Tomato Manager needs storage access to save config backups to:',
            ...
            Container(
              ...
              child: Row(children: [
                Icon(Icons.folder_rounded, color: v.accent, size: 16),
                const SizedBox(width: 8),
                Expanded(child: Text(
                  '/Download/Tomato Manager/Backup/',
                  style: TextStyle(fontFamily: 'monospace', fontSize: 12, color: v.accent),
                )),
              ]),
            ),
            const SizedBox(height: 10),
            const Text(
              'Backup files can be accessed directly from your phone\'s File Manager.',
      ...
      await showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          title: const Row(children: [
            ...
            SizedBox(width: 10),
            Text('Permission Blocked', style: TextStyle(fontSize: 16)),
          ]),
          content: const Text(
            'Storage permission was permanently denied.\n\n'
            'Go to App Settings → Permissions → Storage and enable it manually.',
            style: TextStyle(fontSize: 13, height: 1.5),
          ),
          actions: [
            TextButton(
  ...
  Future<void> _restore() async {
    ...
    if (!fileName.endsWith('.cfg')) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please select a .cfg file'), backgroundColor: V.err));
      return;
    }
    ...
    final ok = await showDialog<bool>(context: context, builder: (_) => AlertDialog(
      ...
      content: Text(
        'Restore settings from "$fileName"?\n\n'
        'This will overwrite all current router settings.'),
      actions: [
    ...
    try {
      ...
      // Step 3: Serve file over HTTP — router wget downloads it directly.
      // This is binary-perfect: no base64, no shell escaping, no encoding issues.
      server = await HttpServer.bind(InternetAddress.anyIPv4, 0);
      final port = server.port;
      final ip   = await _localIp();
      if (ip == null) throw Exception('Could not determine local IP address');

      // Serve exactly once then close
      server.listen((req) async {
      ...
      // Step 4: Router wget the file — pure binary, no conversion
      final dlErr = await ssh.runLong(
        'wget -O /tmp/configuration.cfg http://$ip:$port/',
        timeout: const Duration(minutes: 5));
      if (dlErr.toLowerCase().contains('error') || dlErr.toLowerCase().contains('failed')) {
        throw Exception('Router could not download file: $dlErr');
      }

      // Step 5: nvram restore + commit
      await ssh.run('nvram restore /tmp/configuration.cfg');
      await ssh.run('nvram commit');
      await ssh.run('rm -f /tmp/configuration.cfg');

      if (!mounted) return;

      // Step 5: Offer reboot
      final reboot = await showDialog<bool>(context: context, builder: (_) => AlertDialog(
        title: const Text('Config Restored'),
        content: const Text(
          'Settings have been restored successfully.\n\n'
          'A reboot is required for all changes to take effect. Reboot now?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text('Later')),
          ElevatedButton.icon(
            style: ElevatedButton.styleFrom(backgroundColor: V.warn, foregroundColor: Colors.black),
            icon: const Icon(Icons.restart_alt_rounded, size: 16),
            label: const Text('Reboot Now', style: TextStyle(fontWeight: FontWeight.w700)),
            onPressed: () => Navigator.pop(context, true)),
        ]));

      if (reboot == true) {
        await ssh.run('reboot 2>/dev/null || true');
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Rebooting…'), backgroundColor: V.warn));
      } else {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(
        ...
        VCard(padding: const EdgeInsets.all(14), child: Column(children: [
          ...
          _Row(icon: Icons.refresh_rounded, color: V.info,
            ...
            onTap: () async {
              ...
              if (err == null) {
                ref.read(sshServiceProvider).connectIfNeeded(cfg).catchError((_) {});
