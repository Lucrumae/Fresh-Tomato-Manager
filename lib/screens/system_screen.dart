...
class _RouterTabState extends ConsumerState<_RouterTab> {
  ...
  Future<void> _firmwareUpgrade(BuildContext ctx) async {
    ...
    try {
      ...
      // 1. Download firmware — runLong with 10min timeout, no 2>/dev/null
      //    so wget errors are visible and partial downloads are caught.
      ...
      if (wgetOut.toLowerCase().contains('error')
        ...
        throw Exception('wget failed: $wgetOut');
      }

      // 2. Verify: size on router must exactly match local file
      final sizeOut    = await ssh.run('wc -c < /tmp/firmware.trx 2>/dev/null || echo 0');
      final remoteSize = int.tryParse(sizeOut.trim()) ?? 0;
      if (remoteSize != bytes.length) {
        await ssh.run('rm -f /tmp/firmware.trx');
        throw Exception(
          'Size mismatch — download incomplete. '
          'Expected ${bytes.length}B, got ${remoteSize}B. Aborting flash.');
      }

      // 3. Flash firmware
      await ssh.runLong('mtd-write2 /tmp/firmware.trx linux',
          timeout: const Duration(minutes: 5));
      // 4. Erase nvram so new firmware starts clean
      await ssh.run('nvram erase');
      // 5. Reboot
