...
class _RouterTabState extends ConsumerState<_RouterTab> {
  ...
  Future<void> _firmwareUpgrade(BuildContext ctx) async {
    ...
    try {
      ...
      // 1. Download firmware to router
      await ssh.run('wget -q -O /tmp/firmware.trx http://$ip:$port/ 2>/dev/null');

      // 2. Verify downloaded file matches expected size
      ...
      if (remoteSize == 0) {
        ...
        throw Exception('Download failed — file is empty on router');
      }
      if (remoteSize != bytes.length) {
        await ssh.run('rm -f /tmp/firmware.trx');
        throw Exception('Size mismatch: expected ${bytes.length}B, got ${remoteSize}B');
      }

      // 3. Flash: try mtd-write2 first, fallback to mtd write
      await ssh.run(
        'mtd-write2 /tmp/firmware.trx linux 2>/dev/null'
  ...
  Future<void> _restore() async {
    ...
    try {
      ...
      final dlErr = await ssh.run(
        'wget -q -O /tmp/configuration.cfg http://$ip:$port/ 2>&1');
      if (dlErr.contains('ERROR') || dlErr.contains('failed')) {
