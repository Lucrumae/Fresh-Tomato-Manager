...
class ConnectionKeeper {
  ...
  Future<void> _tick() async {
    ...
    try {
      ...
      final monitorSsh = _ref.read(monitorSshServiceProvider);
      if (!ssh.isConnected) {
        await _reconnect();
      } else if (!monitorSsh.isConnected) {
        // Main session OK but monitor session dropped — reconnect monitor only
        final cfg = _ref.read(configProvider);
        if (cfg != null) {
          debugPrint('[Keeper] Monitor session dropped, reconnecting…');
          await monitorSsh.connectIfNeeded(cfg).timeout(
  ...
  Future<void> _reconnect() async {
    ...
    debugPrint('[Keeper] Reconnecting main session…');
    final err = await ssh.connectIfNeeded(cfg).timeout(
      const Duration(seconds: 12), onTimeout: () => 'timeout');

    if (err == null) {
      debugPrint('[Keeper] Main session reconnected OK');
      // Also reconnect monitor session
      final monitorSsh = _ref.read(monitorSshServiceProvider);
      if (!monitorSsh.isConnected) {
        debugPrint('[Keeper] Reconnecting monitor session…');
        await monitorSsh.connectIfNeeded(cfg).timeout(
          const Duration(seconds: 12), onTimeout: () => 'timeout');
      }
      _restartPollers();
  ...
  void _restartPollers() {
    try {
      ...
      // Clear logs on reconnect so fresh SSH session starts clean
      _ref.read(logsProvider.notifier).clear();
