...
class SshService {
  ...
  // 1. Encode to PEM base64 in Dart.
  // 2. Send as heredoc via ONE run() call — no loops.
  // 3. openssl decode → binary, dd conv=notrunc force-writes binary to dest.
  // Returns null on success, error string on failure.
  Future<String?> uploadBytes(Uint8List bytes, String remotePath) async {
    try {
      ...
      for (var i = 0; i < b64Raw.length; i += 64) {
        buf.writeln(b64Raw.substring(i, (i + 64).clamp(0, b64Raw.length)));
      }

      // Write via heredoc — ONE ssh command, no loops, no quoting issues.
      // Dart string concatenation keeps the newline literal unambiguous.
      final cmd = "cat > /tmp/cfg_upload.b64 << 'HEOF'" + "\n"
          + buf.toString()
          + "HEOF";
      final writeErr = await run(cmd);
      if (writeErr.trim().isNotEmpty) {
        await run('rm -f /tmp/cfg_upload.b64');
        return 'Write error: $writeErr';
      }

      // Decode base64 to binary, then dd conv=notrunc force-writes as binary.
      final decErr = await run(
        'openssl enc -base64 -d -in /tmp/cfg_upload.b64 -out /tmp/cfg_upload.bin'
        ' && dd if=/tmp/cfg_upload.bin of="' + remotePath + '" conv=notrunc 2>/dev/null'
        ' && rm -f /tmp/cfg_upload.b64 /tmp/cfg_upload.bin'
      );
      if (decErr.contains('error') || decErr.contains('Error')) {
