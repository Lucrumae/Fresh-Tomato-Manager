...
class SshService {
  SSHClient?  _client;
  SSHSession? _shell;         // single persistent /bin/sh session
  TomatoConfig? _config;
  bool _connecting = false;

  // Shell stdout buffer — bytes from the persistent shell accumulated here
  final StringBuffer _shellBuf = StringBuffer();
  StreamSubscription<List<int>>? _shellSub;

  // Command queue — serialised, 1 in-flight at a time
  bool _cmdInFlight = false;
  final _cmdQueue   = <_SshCmd>[];

  bool get isConnected =>
      _client != null && !(_client!.isClosed) && _shell != null;
  SSHClient? get client => _client;

  // ── Connect ───────────────────────────────────────────────────────────────
  // Returns null on success, error string on failure.
  // Safe to call even if already connected — reuses existing session.
  Future<String?> connectIfNeeded(TomatoConfig config) async {
    if (isConnected) return null; // already up, reuse
  ...
  Future<String?> connect(TomatoConfig config) async {
    ...
    _config = config;
    try {
      // Only close existing if it's actually dead — don't force-close a live session
      if (_client != null && _client!.isClosed) await disconnect();
      if (_shell != null && _client != null && !_client!.isClosed) {
        // Shell died but client is alive — just reopen the shell
        try {
          _shellSub?.cancel(); _shellSub = null;
          try { _shell?.close(); } catch (_) {}
          _shell = null;
          _shellBuf.clear();
          _shell = await _client!.shell(
            pty: const SSHPtyConfig(type: 'dumb', width: 220, height: 50),
          );
          _shell!.stdin.add(Uint8List.fromList('stty -echo 2>/dev/null; PS1=\'\'; export PS1\n'.codeUnits));
          _shellSub = _shell!.stdout.listen(
            (bytes) => _shellBuf.write(String.fromCharCodes(bytes)),
            onDone: _onShellClosed, onError: (_) => _onShellClosed(),
          );
          ...
          // Shell reopen failed — do full reconnect below
          await disconnect();
        }
      } else {
        await disconnect();
      ...
      // Open ONE persistent shell — all commands run through this single channel
      _shell = await _client!.shell(
        pty: const SSHPtyConfig(type: 'dumb', width: 220, height: 50),
      );

      // Disable terminal echo + clear prompt so output isn't polluted
      _shellBuf.clear();
      _shell!.stdin.add(Uint8List.fromList('stty -echo 2>/dev/null; PS1=\'\'; export PS1\n'.codeUnits));

      // Subscribe to stdout — stream into buffer
      _shellSub = _shell!.stdout.listen(
        (bytes) => _shellBuf.write(String.fromCharCodes(bytes)),
        onDone:  _onShellClosed,
        onError: (_) => _onShellClosed(),
      );

      // Wait for shell init noise to settle, then clear buffer
      await Future.delayed(const Duration(milliseconds: 400));
      _shellBuf.clear();
  ...
  void _onShellClosed() {
    debugPrint('[SSH] Shell closed');
    _shell = null;
    for (final cmd in _cmdQueue) cmd.completer.completeError(Exception('Shell closed'));
    _cmdQueue.clear();
    _cmdInFlight = false;
    // Auto-recover: try to reopen shell immediately without a full TCP reconnect
    if (_config != null && _client != null && !_client!.isClosed) {
      debugPrint('[SSH] Attempting shell reopen…');
      Future.delayed(const Duration(seconds: 1), () => connect(_config!));
    }
  }
  ...
  Future<void> disconnect() async {
    _connecting = false;
    _shellSub?.cancel(); _shellSub = null;
    for (final cmd in _cmdQueue) cmd.completer.completeError(Exception('Disconnected'));
    _cmdQueue.clear();
    _cmdInFlight = false;
    try { _shell?.close(); } catch (_) {}
    _shell = null;
  ...
  // ── Run command ───────────────────────────────────────────────────────────
  // All commands are serialised through the single persistent shell.
  ...
  // Like run() but with a longer timeout — use for wget/flash/heavy operations.
  Future<String> runLong(String command, {Duration timeout = const Duration(minutes: 5)}) {
    if (!isConnected) return Future.error(Exception('Not connected'));
    final cmd = _SshCmd(command, timeout: timeout);
    _cmdQueue.add(cmd);
    _drainQueue();
    return cmd.completer.future;
  }
  ...
  void _drainQueue() {
    ...
    // Drop oldest stale poll commands when backing up
    while (_cmdQueue.length > 3) _cmdQueue.removeAt(0).completer.complete('');
    _cmdInFlight = true;
    final cmd = _cmdQueue.removeAt(0);
    _execShell(cmd.command, timeout: cmd.timeout)
      .then(cmd.completer.complete)
      .catchError(cmd.completer.completeError)
      .whenComplete(() { _cmdInFlight = false; _drainQueue(); });
  }

  // Execute one command through the persistent shell.
  // Wraps command as:  { <cmd> ; } 2>&1; printf "%s" "<sentinel>"
  // Then polls buffer until the sentinel appears.
  Future<String> _execShell(String command, {Duration timeout = const Duration(seconds: 15)}) async {
    if (_shell == null || _client == null || _client!.isClosed) {
      throw Exception('Not connected');
    }
    // Unique sentinel per command — rare hex string, always at end of output
    final id       = DateTime.now().microsecondsSinceEpoch.toRadixString(16);
    final sentinel = '__SSHEND_${id}__';

    _shellBuf.clear();
    // Use single-quoted sentinel in printf to avoid shell variable expansion
    _shell!.stdin.add(Uint8List.fromList("{ $command ; } 2>&1; printf '%s' '$sentinel'\n".codeUnits));

    const interval = Duration(milliseconds: 20);
    final deadline = DateTime.now().add(timeout);

    while (DateTime.now().isBefore(deadline)) {
      final buf = _shellBuf.toString();
      final idx = buf.indexOf(sentinel);
      if (idx >= 0) {
        final output = buf.substring(0, idx).trim();
        final after  = buf.substring(idx + sentinel.length);
        _shellBuf.clear();
        if (after.isNotEmpty) _shellBuf.write(after);
        return output;
      }
  ...
  Future<String> runWithStderr(String command) => run(command);
  Future<String> _runWithStderrLegacy(String command) => run(command);
  ...
  Future<RouterStatus> getStatus() async {
    try {
      ...
      // Wireless phy temperature — try phy_tempsense_reading first, fallback phy_tempsense
      final wlTempRaw  = await run(
        'wl -i eth1 phy_tempsense_reading 2>/dev/null || wl -i eth1 phy_tempsense 2>/dev/null || echo 0; '
        'wl -i eth2 phy_tempsense_reading 2>/dev/null || wl -i eth2 phy_tempsense 2>/dev/null || echo 0'
      );
  ...
  RouterStatus _parseStatus(String raw) {
    try {
      ...
      //   "68"          — raw integer (Broadcom raw, apply /2+20)
      //   "68 255"      — raw + flags (phy_tempsense_reading)
      //   "54 (0x36)"   — already °C on some wl versions
      //   "temperature: 54" — labelled output
      double wifiTemp24 = 0, wifiTemp5 = 0;
      for (final entry in [(wlt.length >= 1 ? wlt[0] : ''), (wlt.length >= 2 ? wlt[1] : '')].asMap().entries) {
        final t = entry.value.trim();
        if (t.isEmpty || t == '0') continue;
        double temp = 0;
        // "temperature: N" or "temp: N"
        final lm = RegExp(r'temp(?:erature)?[\s:]+(\d+)', caseSensitive: false).firstMatch(t);
        if (lm != null) {
          temp = double.tryParse(lm.group(1)!) ?? 0;
        } else {
          final first = int.tryParse(t.split(RegExp(r'[\s(,]+')).first) ?? 0;
          if (first > 0) {
            // If already in plausible °C range, use directly; else apply Broadcom formula
            temp = (first >= 20 && first <= 120) ? first.toDouble() : first / 2.0 + 20;
          }
        }
        if (entry.key == 0) wifiTemp24 = temp;
        else wifiTemp5 = temp;
      }
      ...
      return RouterStatus(
        ...
        wifiSecurity5:  sec5,
        wifiTxpower24:  (txpwr24.isNotEmpty && txpwr24 != '0') ? '${txpwr24} mW' : 'auto',
        wifiTxpower5:   (txpwr5.isNotEmpty  && txpwr5  != '0') ? '${txpwr5} mW' : 'auto',
  ...
  // ── Single-command fast poll: CPU + RAM + temp + bandwidth in ONE round-trip ──
  // Replaces separate getStatusFast() + getBandwidthRaw() calls.
  // Uses section markers so output can be split and parsed independently.
  Future<({RouterStatus status, Map<String,int> bandwidth})> getCombinedFast(
      RouterStatus current) async {
    try {
      final raw = await run(
      ...
      // ── Split sections ────────────────────────────────────────────────────
      String secCpu = '', secMem = '', secTemp = '', secWlTemp = '', secBw = '';
      String? cur;
      final sb = StringBuffer();
      for (final line in raw.split('\n')) {
        switch (line.trim()) {
          case '==CPU==':    cur = 'cpu';    sb.clear(); continue;
          case '==MEM==':    if (cur == 'cpu')    { secCpu    = sb.toString().trim(); } cur = 'mem'; sb.clear(); continue;
          case '==TEMP==':   if (cur == 'mem')    { secMem    = sb.toString().trim(); } cur = 'tmp'; sb.clear(); continue;
          case '==WLTEMP==': if (cur == 'tmp')    { secTemp   = sb.toString().trim(); } cur = 'wlt'; sb.clear(); continue;
          case '==BW==':     if (cur == 'wlt')    { secWlTemp = sb.toString().trim(); } cur = 'bw';  sb.clear(); continue;
        }
        if (cur != null) { sb.write(line); sb.write('\n'); }
      }
      if (cur == 'bw')  secBw     = sb.toString().trim();
      if (cur == 'wlt') secWlTemp = sb.toString().trim();

      // ── Parse CPU ─────────────────────────────────────────────────────────
      double cpuPercent = current.cpuPercent;
      final cpuParts = secCpu.split(RegExp(r'\s+'));
      if (cpuParts.length >= 5) {
        final jiffies = cpuParts.skip(1).take(8)
            .map((s) => int.tryParse(s) ?? 0).toList();
        if (_prevCpuJiffies.length == jiffies.length) {
          final deltas = List.generate(jiffies.length,
              (i) => jiffies[i] - _prevCpuJiffies[i]);
          // Guard: if any counter went backwards (reboot/wrap), skip this sample
          final anyNegative = deltas.any((d) => d < 0);
          if (!anyNegative) {
            final idleDelta  = deltas[3] + (deltas.length > 4 ? deltas[4] : 0);
            final totalDelta = deltas.fold(0, (a, b) => a + b);
            if (totalDelta > 0) cpuPercent = (1 - idleDelta / totalDelta) * 100;
      ...
      // ── Parse RAM ─────────────────────────────────────────────────────────
      // Keep previous values if section is empty (parse failed / SSH glitch)
      int memTotal = current.ramTotalMB * 1024;
      int memFree = 0, memBuffers = 0, memCached = 0;
      bool memParsed = false;
      for (final line in secMem.split('\n')) {
        final p = line.trim().split(RegExp(r'\s+'));
        if (p.length >= 2) {
          final val = int.tryParse(p[1]) ?? 0;
          if (line.startsWith('MemTotal'))  { memTotal   = val; memParsed = true; }
          if (line.startsWith('MemFree'))     memFree    = val;
          if (line.startsWith('Buffers'))     memBuffers = val;
          if (line.startsWith('Cached') && !line.startsWith('SwapCached')) memCached = val;
        }
      }
      // If parse failed, keep previous RAM values to avoid spurious 0 spikes
      final memUsed = memParsed
          ? (memTotal - memFree - memBuffers - memCached).clamp(0, memTotal)
          : current.ramUsedMB * 1024;

      // ── Parse temp ────────────────────────────────────────────────────────
      // Keep previous value if section is empty or parse fails
      double cpuTempC = current.cpuTempC;
      if (secTemp.isNotEmpty) {
        final tempStr = secTemp.split('\n').first.replaceAll(RegExp(r'[^0-9.]'), '').trim();
        final tempVal = double.tryParse(tempStr) ?? 0;
        if (tempVal > 1000) cpuTempC = tempVal / 1000;
        else if (tempVal > 0) cpuTempC = tempVal;
      }

      // ── Parse wireless temperatures ──────────────────────────────────────────
      double wifiTemp24 = current.wifiTemp24;
      double wifiTemp5  = current.wifiTemp5;
      final wltLines = secWlTemp.split('\n').where((l) => l.trim().isNotEmpty).toList();
      for (final entry in wltLines.asMap().entries) {
        final t = entry.value.trim();
        if (t.isEmpty || t == '0') continue;
        double temp = 0;
        final lm = RegExp(r'temp(?:erature)?[\s:]+(\d+)', caseSensitive: false).firstMatch(t);
        if (lm != null) {
          ...
          final first = int.tryParse(t.split(RegExp(r'[\s(,]+')).first) ?? 0;
          if (first > 0) temp = (first >= 20 && first <= 120) ? first.toDouble() : first / 2.0 + 20;
        }
        if (entry.key == 0 && temp > 0) wifiTemp24 = temp;
        if (entry.key == 1 && temp > 0) wifiTemp5  = temp;
      }

      // ── Parse bandwidth ───────────────────────────────────────────────────
      String wanIface = current.wanIface ?? '';
      final ifaceBytes = <String, Map<String,int>>{};
      for (final line in secBw.split('\n')) {
        if (line.startsWith('=IFACE=')) {
          wanIface = line.substring(7).trim();
          continue;
        }
        final t = line.trim();
      ...
      // Pick WAN iface (same logic as getBandwidthRaw)
      Map<String,int> bwResult = {'rx': 0, 'tx': 0};
      if (wanIface.isNotEmpty && ifaceBytes.containsKey(wanIface)) {
        bwResult = ifaceBytes[wanIface]!;
      } else {
        for (final cand in _wanCandidates) {
          final match = ifaceBytes.keys
              .where((k) => k.startsWith(cand) &&
                  !_skipIfaces.any((s) => k.startsWith(s)))
              .toList()..sort();
          if (match.isNotEmpty) { bwResult = ifaceBytes[match.first]!; break; }
        }
      }

      final status = RouterStatus(
        cpuPercent:    cpuPercent.clamp(0.0, 100.0),
        ramUsedMB:     (memUsed / 1024).round(),
        ramTotalMB:    (memTotal / 1024).round(),
        uptime:        current.uptime,
        ...
        isOnline:      true,
        cpuTempC:      cpuTempC,
        wanIface:      wanIface.isNotEmpty ? wanIface : current.wanIface,
        wifiTemp24:    wifiTemp24,
        wifiTemp5:     wifiTemp5,
      );

      return (status: status, bandwidth: bwResult);
    } catch (_) {
      return (status: current, bandwidth: {'rx': 0, 'tx': 0});
    }
  }
  ...
  Future<List<ConnectedDevice>> getDevices() async {
    try {
      final raw = await run(
        'echo "=WAN="; '
        'nvram get wan_ipaddr 2>/dev/null || echo ""; '
        'nvram get wan_gateway 2>/dev/null || echo ""; '
        'nvram get wan_hwaddr 2>/dev/null || echo ""; '
        'nvram get wan_iface 2>/dev/null || echo ""; '
        'echo "=ARP="; '
      ...
      String section = '';
      String lanPrefix = '192.168.'; // fallback
      String wanIp = '', wanGateway = '', wanMac = '';
      final wanLines = <String>[];
      for (final line in raw.split('\n')) {
        final t = line.trim();
        if (t == '=WAN=' || t == '=ARP=' || t == '=LEASES=' || t == '=LAN=' || t == '=WIFI=') { section = t; continue; }
        if (t.isEmpty) continue;

        if (section == '=WAN=') {
          wanLines.add(t);
        } else if (section == '=LAN=') {
          // e.g. "192.168.1.1" — extract subnet prefix for WAN filtering
          final parts = t.split('.');
          if (parts.length == 4) lanPrefix = '${parts[0]}.${parts[1]}.${parts[2]}.';
      ...
      // Extract WAN fields from wanLines (order: ip, gateway, mac, iface)
      if (wanLines.length >= 1) wanIp      = wanLines[0];
      if (wanLines.length >= 2) wanGateway = wanLines[1];
      if (wanLines.length >= 3) wanMac     = wanLines[2].toLowerCase();
      String wanIfaceName = wanLines.length >= 4 ? wanLines[3] : '';
      if (wanIfaceName.isEmpty) wanIfaceName = 'wan';

      // Parse ARP table
      final devices  = <ConnectedDevice>[];
      final seenMacs = <String>{};

      // Add WAN gateway as a special device entry
      if (wanGateway.isNotEmpty && wanGateway != '0.0.0.0') {
        final mac = wanMac.isNotEmpty ? wanMac : 'wan:00:00:00:00:00';
        seenMacs.add(mac);
        devices.add(ConnectedDevice(
          ip:        wanGateway,
          mac:       mac,
          name:      'WAN Gateway',
          hostname:  wanIp.isNotEmpty ? 'Public IP: $wanIp' : '',
          interface: wanIfaceName,
          rssi:      '',
          isBlocked: false,
      ...
      for (final line in raw.split('\n')) {
        ...
        // Skip WAN-side entries (e.g. gateway on usb0, not on LAN subnet)
        if (!ip.startsWith(lanPrefix)) continue;
        seenMacs.add(mac);
  ...
  Future<List<LogEntry>> getLogs() async {
    try {
      // Fetch syslog + dmesg separately, merge them
      // /var/log/messages = runtime syslog (kern+system, syslog format)
      // dmesg = kernel ring buffer (boot messages, plain text, no syslog prefix)
      final raw = await run(
        'cat /var/log/messages 2>/dev/null || logread 2>/dev/null || echo ""'
      );
      final dmesgRaw = await run('dmesg 2>/dev/null || echo ""');
      if (raw.trim().isEmpty) return [];

      // BusyBox syslog format: "Jan  1 07:00:03 unknown kern.notice kernel: message"
      // Bug fix 1: old regex (\S+?)(?:\[\d+\])?:  fails on "syslogd started:" (space before colon)
      //            fixed: use (.*?) to capture full process+args up to optional [pid] then colon
      // Bug fix 2: DateTime.parse() doesn't understand syslog timestamps — use manual parser
      final syslogRe = RegExp(
        r'^(\w{3}\s+\d+\s+\d+:\d+:\d+)\s+\S+\s+(\S+?)\.(\w+)\s+(.*?)(?:\[\d+\])?:\s*(.*)$'
      );
      final simpleSyslogRe = RegExp(
        r'^(\w{3}\s+\d+\s+\d+:\d+:\d+)\s+\S+\s+(.*?)(?:\[\d+\])?:\s*(.*)$'
      );
      // Manual syslog timestamp parser — DateTime.parse() doesn't handle "Jan  1 07:00:03"
      DateTime parseSyslogTime(String ts) {
        try {
          final parts = ts.trim().split(RegExp(r'\s+'));
          if (parts.length < 3) return DateTime.now();
          const months = {'Jan':1,'Feb':2,'Mar':3,'Apr':4,'May':5,'Jun':6,
                          'Jul':7,'Aug':8,'Sep':9,'Oct':10,'Nov':11,'Dec':12};
          final mon = months[parts[0]] ?? 1;
          final day = int.tryParse(parts[1]) ?? 1;
          final hms = parts[2].split(':');
          final h = int.tryParse(hms[0]) ?? 0;
          final m = int.tryParse(hms.length > 1 ? hms[1] : '0') ?? 0;
          final sec = int.tryParse(hms.length > 2 ? hms[2] : '0') ?? 0;
          return DateTime(DateTime.now().year, mon, day, h, m, sec);
      ...
      for (final line in raw.split('\n')) {
        ...
        if (m1 != null) {
          ...
          // group(4) may be "syslogd started" — take only first word as process name
          process  = (m1.group(4) ?? '').trim().split(RegExp(r'\s+')).first;
          message  = m1.group(5) ?? t;
        } else if (m2 != null) {
          timeStr = m2.group(1) ?? '';
          process = (m2.group(2) ?? '').trim().split(RegExp(r'\s+')).first;
        ...
        final time = timeStr.isNotEmpty ? parseSyslogTime(timeStr) : DateTime.now();
      ...
      // Parse dmesg — plain text kernel boot messages, no syslog prefix
      // Format: "Linux version..." / "CPU: ..." / "Memory: ..."
      if (dmesgRaw.trim().isNotEmpty) {
        final bootTime = DateTime(DateTime.now().year, 1, 1); // dmesg has no real timestamp
        for (final line in dmesgRaw.split('\n')) {
          final t = line.trim();
          if (t.isEmpty) continue;
          // Skip lines that are already in /var/log/messages (have syslog prefix)
          if (syslogRe.hasMatch(t) || simpleSyslogRe.hasMatch(t)) continue;
          String level = 'info';
          final lower = t.toLowerCase();
          if (lower.contains('error') || lower.contains('fail') || lower.contains('oops') ||
              lower.contains('panic') || lower.contains('bug:')) level = 'err';
      ...
      return entries; // oldest first → newest last
    } catch (e) {
      return [];
    }
  }
  ...
  Future<List<PortForwardRule>> getPortForwardRules() async {
    try {
      ...
      for (final entry in raw.split('>').where((s) => s.isNotEmpty)) {
        // FreshTomato format: enabled<proto<src_filter<ext_port<int_port<int_ip<desc
        // ext_port can be: "80", "1000:2000" (range), "80,443" (multi), "1000:2000,3000"
        // int_port can be blank (same as ext_port) or a single port "8080"
        final parts = entry.split('<');
        if (parts.length >= 6) {
          final protoNum  = int.tryParse(parts[1].trim()) ?? 3;
          final extPort   = parts[3].trim();
          final intPortRaw = parts[4].trim();
          // blank int_port means same as ext_port
          final intPort   = intPortRaw.isEmpty ? extPort : intPortRaw;
  ...
  Future<bool> savePortForwardRules(List<PortForwardRule> rules) async {
    try {
      final encoded = rules.map((r) {
        ...
        // int_port blank if same as ext_port (FreshTomato convention)
        final intPort = r.internalPort == r.externalPort ? '' : r.internalPort;
        return '${r.enabled ? 1 : 0}<$protoNum<${r.srcFilter}<${r.externalPort}<$intPort<${r.internalIp}<${r.name}';
      }).join('>');
...
class _SshCmd {
  ...
  _SshCmd(this.command, {this.timeout = const Duration(seconds: 15)});
}