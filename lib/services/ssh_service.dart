...
class SshService {
  ...
  Future<String> _execShell(String command) async {
    if (_shell == null || _client == null || _client!.isClosed) {
  ...
  // ── Upload binary file to router ──────────────────────────────────────────
  // Pipe base64-encoded data directly into openssl via stdin — no temp .b64
  // file, no shell redirection quoting issues, no batch loops.
  // Each chunk: echo <b64chunk> | openssl enc -base64 -d >> remotePath
  // openssl reads stdin when -in is omitted; -d decodes; output appended.
  // Chunk size 48 bytes raw = 64 chars base64 (one PEM line) — tiny but
  // we batch many lines per run() call to keep round-trips low.
  // Returns null on success, error string on failure.
  Future<String?> uploadBytes(Uint8List bytes, String remotePath) async {
    try {
      // Encode entire file as PEM base64 (64-char lines)
      final b64Raw = base64Encode(bytes);
      final lines = <String>[];
      for (var i = 0; i < b64Raw.length; i += 64) {
        lines.add(b64Raw.substring(i, (i + 64).clamp(0, b64Raw.length)));
      }

      // Write all lines as a heredoc to a temp .b64 file.
      // We use a Python-style approach: build one big shell script string
      // that uses 'cat << EOF' which avoids ALL redirection quoting problems.
      // Split into batches to stay under shell command length limits.
      const linesPerBatch = 50; // 50 lines × 64 chars = 3200 chars per batch

      for (var i = 0; i < lines.length; i += linesPerBatch) {
        final batch = lines.sublist(i, (i + linesPerBatch).clamp(0, lines.length));
        final op = i == 0 ? '>' : '>>';
        // Use printf with explicit \n — one printf call per line joined with ;
        // This avoids the >> inside join bug entirely by using a single
        // compound command with the redirection on the outside.
        final printfs = batch.map((l) => 'printf "%s\\n" "$l"').join('; ');
        final cmd = '{ $printfs; } $op "/tmp/configuration.cfg.b64"';
        final result = await run(cmd);
        if (result.trim().isNotEmpty) {
          await run('rm -f "/tmp/configuration.cfg.b64"');
          return 'Write error at batch ${i ~/ linesPerBatch}: $result';
        }
      }

      // Decode the complete PEM base64 file to binary in one step.
      // openssl enc -base64 -d reads PEM-formatted input natively.
      final decErr = await run(
        'openssl enc -base64 -d -in "/tmp/configuration.cfg.b64" -out "$remotePath"'
        ' && rm -f "/tmp/configuration.cfg.b64"');
      if (decErr.trim().isNotEmpty) {
        await run('rm -f "/tmp/configuration.cfg.b64" "$remotePath"');
        return 'Decode error: $decErr';
      }
  ...
  RouterStatus _parseStatus(String raw) {
    try {
      ...
      // Output format: '121 255' or '121' — take only first token
      double wifiTemp24 = 0, wifiTemp5 = 0;
      if (wlt.length >= 1) {
        final raw24 = int.tryParse(wlt[0].trim().split(RegExp(r'\s+')).first) ?? 0;
        if (raw24 > 0) wifiTemp24 = raw24 / 2.0 + 20;
      }
      if (wlt.length >= 2) {
        final raw5 = int.tryParse(wlt[1].trim().split(RegExp(r'\s+')).first) ?? 0;
  ...
  Future<({RouterStatus status, Map<String,int> bandwidth})> getCombinedFast(
    ...
    try {
      ...
      // ── Split sections ────────────────────────────────────────────────────
      String secCpu = '', secMem = '', secTemp = '', secBw = '';
      String? cur;
      ...
      for (final line in raw.split('\n')) {
        switch (line.trim()) {
          ...
          case '==BW==':   if (cur == 'tmp') { secTemp = sb.toString().trim(); } cur = 'bw';  sb.clear(); continue;
        }
  ...
  Future<List<ConnectedDevice>> getDevices() async {
    try {
      ...
      for (final line in raw.split('\n')) {
        final t = line.trim();
        if (t == '=ARP=' || t == '=LEASES=' || t == '=LAN=' || t == '=WIFI=') { section = t; continue; }
        if (t.isEmpty) continue;
