...
class SshService {
  ...
  // ── Upload binary file to router via base64-encoded shell command ──────────
  // Encodes entire file as one base64 string, writes to a temp .b64 file on
  // the router, then decodes it — avoids all shell quoting and echo/printf
  // issues entirely. Clean binary guaranteed.
  // Returns null on success, error string on failure.
  Future<String?> uploadBytes(Uint8List bytes, String remotePath) async {
    try {
      final b64File = '$remotePath.b64';
      const chunkSize = 3072; // 3KB per chunk — safe for BusyBox arg limits
      final b64 = base64Encode(bytes);

      // Write base64 string in chunks to a .b64 temp file on the router
      // Use printf with %s and no quoting issues — base64 chars are [A-Za-z0-9+/=]
      // which are all safe; no shell special chars possible in base64 output.
      for (var i = 0; i < b64.length; i += chunkSize) {
        final chunk = b64.substring(i, (i + chunkSize).clamp(0, b64.length));
        final op    = i == 0 ? '>' : '>>';
        final result = await run('printf "%s" "$chunk" $op "$b64File"');
        if (result.trim().isNotEmpty) {
          await run('rm -f "$b64File"');
          return 'Write error at chunk ${i ~/ chunkSize}: $result';
        }
      }

      // Decode the .b64 file into the final binary — base64 -d reads a file,
      // no shell involvement, guaranteed clean binary output.
      // openssl enc -base64 -d is available on all Tomato firmware builds.
      // 'base64' command is NOT available on BusyBox/Tomato — never use it.
      // -A flag: accept input without line breaks (our b64 string has none).
      final decodeResult = await run(
        'openssl enc -base64 -d -A -in "$b64File" -out "$remotePath" && rm -f "$b64File"');
