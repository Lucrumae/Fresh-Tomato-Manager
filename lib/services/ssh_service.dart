...
class SshService {
  ...
  Future<Map<String, dynamic>> getTrafficHistory() async {
    try {
      ...
      final raw = await run(
        'kill -USR1 \$(cat /var/run/rstats.pid 2>/dev/null) 2>/dev/null; '
        'sleep 1; '
        'echo "=MONTHS="; '
  ...
  Future<bool> saveWifiSettings({
    ...
    try {
      ...
      if (channel  != null && channel.isNotEmpty) cmds.add("nvram set ${iface}_channel='$channel'");
      if (security != null && security.isNotEmpty) {
        cmds.add("nvram set ${iface}_security_mode='$security'");
        if (security == 'disabled') {
          cmds.add("nvram set ${iface}_crypto=''");
        } else if (security.contains('wpa2')) {
          cmds.add("nvram set ${iface}_crypto='aes'");
        }
      }
      if (txpower  != null && txpower.isNotEmpty) cmds.add("nvram set ${iface}_txpwr='$txpower'");
      if (cmds.isEmpty) return true;
      cmds.add('nvram commit');
      // Apply without full restart — soft apply
      cmds.add('wl -i ${iface == "wl0" ? "eth1" : "eth2"} ssid "\${nvram get ${iface}_ssid}" 2>/dev/null; true');
      await run(cmds.join('; '));
      return true;
    } catch (e) {
