...
class SshService {
  ...
  // Sends the entire file as a single SSH command using a shell heredoc.
  // No loops, no batching, no temp files beyond the .b64 stage.
  // Step 1: Write all base64 to /tmp/x.b64 in ONE heredoc command.
  // Step 2: openssl dec -base64 -d to get binary.
  // Step 3: dd conv=notrunc to force-write as binary to final path.
  // Returns null on success, error string on failure.
  Future<String?> uploadBytes(Uint8List bytes, String remotePath) async {
    try {
      // Encode to PEM base64 (64-char lines) — openssl native format
      final b64Raw = base64Encode(bytes);
      final b64Lines = StringBuffer();
      for (var i = 0; i < b64Raw.length; i += 64) {
        b64Lines.writeln(b64Raw.substring(i, (i + 64).clamp(0, b64Raw.length)));
      }
      final b64Content = b64Lines.toString();

      // Write entire base64 to temp file using a single heredoc.
      // HEREDOC_END is unlikely to appear in base64 output.
      // This is ONE run() call — no loop.
      final writeResult = await run(
        'cat > /tmp/cfg_upload.b64 << 'HEREDOC_END'
'
        '$b64Content'
        'HEREDOC_END'
      );
      if (writeResult.trim().isNotEmpty) {
        await run('rm -f /tmp/cfg_upload.b64');
        return 'Write error: $writeResult';
      }

      // Decode base64 → binary, then dd conv=notrunc to force binary write
      final decodeResult = await run(
        'openssl enc -base64 -d -in /tmp/cfg_upload.b64 > /tmp/cfg_upload.bin'
        ' && dd if=/tmp/cfg_upload.bin of="$remotePath" conv=notrunc 2>/dev/null'
        ' && rm -f /tmp/cfg_upload.b64 /tmp/cfg_upload.bin'
      );
      if (decodeResult.contains('error') || decodeResult.contains('Error')) {
        await run('rm -f /tmp/cfg_upload.b64 /tmp/cfg_upload.bin');
        return 'Decode error: $decodeResult';
      }
