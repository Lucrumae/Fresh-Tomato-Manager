...
class SshService {
  ...
  Future<List<LogEntry>> getLogs() async {
    try {
      // logread is the standard FreshTomato/OpenWRT way to read syslog.
      // /var/log/messages often exists but is empty — using subshell so an empty
      // file doesn't prevent fallback commands from running.
      final raw = await run(
        '{ logread 2>/dev/null || cat /var/log/messages 2>/dev/null || dmesg 2>/dev/null; }'
        ' | tail -500'
      );
      if (raw.trim().isEmpty) return [];

      // FreshTomato / BusyBox syslog formats:
      //   "Jan  1 07:00:03 (none) kern.notice kernel: msg"         (standard)
      //   "Jan  1 07:00:03 2024 (none) kern.notice kernel: msg"    (with year)
      //   "Tue Jan  1 07:00:03 2024 (none) kern.notice kernel: msg"(with dow+year)
      // Captures: 1=timestamp  2=facility  3=level  4=process  5=message
      final p1 = RegExp(
        r'^(?:\w{3}\s+)?(\w{3}\s+\d+\s+\d+:\d+:\d+)(?:\s+\d{4})?\s+\S+\s+(\S+?)\.(\w+)\s+(\S+?)(?:\[\d+\])?:\s*(.*)$'
      );
      // Simple format without facility: "Jan  1 07:00:03 (none) process: msg"
      final p2 = RegExp(
        r'^(?:\w{3}\s+)?(\w{3}\s+\d+\s+\d+:\d+:\d+)(?:\s+\d{4})?\s+\S+\s+(\S+?)(?:\[\d+\])?:\s*(.*)$'
      );
      // Raw dmesg: "[  123.456] message"
      final p3 = RegExp(r'^\[\s*\d+\.\d+\]\s+(.*)$');
      ...
      for (final line in raw.split('\n')) {
        ...
        final m1 = p1.firstMatch(t);
        final m2 = m1 == null ? p2.firstMatch(t) : null;
        final m3 = (m1 == null && m2 == null) ? p3.firstMatch(t) : null;
        ...
        if (m1 != null) {
          ...
          // Raw dmesg line — treat as kernel
          process  = 'kernel';
          facility = 'kern';
          message  = m3.group(1) ?? t;
        }

        // source: kernel if facility=kern OR process=kernel/klogd
        final isKernFacility = facility == 'kern';
        final isKernProcess  = process == 'kernel' || process == 'klogd';
        final source = (isKernFacility || isKernProcess) ? 'kernel' : 'system';

        // Map syslog facility.level to severity
        String level = 'info';
        ...
        // Fallback keyword scan when no structured level
        if (levelStr.isEmpty) {
        ...
        if (timeStr.isNotEmpty) {
          try {
            ...
            final withYear = '$clean ${DateTime.now().year}';
            time = DateTime.parse(withYear);
          } catch (_) {
            // Keep DateTime.now() as fallback
          }
        }
      ...
      debugPrint('[SSH] getLogs error: $e');
      return [];
    }
  }
