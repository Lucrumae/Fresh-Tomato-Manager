...
class SshService {
  ...
  // Encodes to PEM base64 (newline every 64 chars), writes in batches using
  // { printf; printf; } >> file pattern (redirection outside the group),
  // then decodes with openssl WITHOUT -A flag — guaranteed binary output.
  Future<String?> uploadBytes(Uint8List bytes, String remotePath) async {
    try {
      final b64File = '$remotePath.b64';

      // Split into 64-char PEM lines — openssl native format, no -A needed
      ...
      // Write 30 lines per SSH call using { cmd; cmd; } > file
      // Redirection is OUTSIDE the braces — no >> inside the join
      const batchSize = 30;
      for (var i = 0; i < lines.length; i += batchSize) {
        final batch  = lines.sublist(i, (i + batchSize).clamp(0, lines.length));
        final op     = i == 0 ? '>' : '>>';
        final inner  = batch.map((l) => 'printf "%s\\n" "$l"').join('; ');
        final result = await run('{ $inner; } $op "$b64File"');
      ...
      // Decode PEM base64 → clean binary. No -A: openssl handles
      // standard 64-char lines natively on ALL versions.
