...
class SshService {
  ...
  // Uses printf '%s' (not echo) to avoid newline/escape injection.
  // Splits into 3KB chunks — conservative limit for BusyBox sh arg length.
  // After upload, strips any stray \r with tr to guarantee clean binary.
  // Returns null on success, error string on failure.
  Future<String?> uploadBytes(Uint8List bytes, String remotePath) async {
    try {
      const chunkSize = 3072; // 3KB — safe for BusyBox shell arg limits

      // Write chunks using printf '%s' — never adds newline or interprets escapes
      for (var i = 0; i < bytes.length; i += chunkSize) {
        final chunk = bytes.sublist(i, (i + chunkSize).clamp(0, bytes.length));
        final b64   = base64Encode(chunk);
        final op    = i == 0 ? '>' : '>>';
        // printf '%s' is POSIX — safe on BusyBox, ash, dash, bash
        final result = await run(
          "printf '%s' '$b64' | base64 -d $op \"$remotePath\"");
        if (result.trim().isNotEmpty) {
          return 'Upload error at chunk ${i ~/ chunkSize}: $result';
        }
      }

      // Ensure clean binary: convert file through dd to strip any corruption,
      // then use tr to remove stray \r bytes that shell might have injected.
      // Write to a temp file first, then atomically replace.
      await run(
        'tr -d "\\r" < "$remotePath" > "${remotePath}.clean" '
        '&& mv "${remotePath}.clean" "$remotePath" '
        '|| rm -f "${remotePath}.clean"'
      );
