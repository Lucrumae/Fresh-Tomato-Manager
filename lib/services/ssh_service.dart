...
class SshService {
  ...
  // Encode to base64 with 64-char line wrapping (PEM format), write lines to
  // a .b64 temp file on the router, decode with openssl (no -A flag needed).
  // This is the most compatible approach across all Tomato/FreshTomato builds.
  // Returns null on success, error string on failure.
  Future<String?> uploadBytes(Uint8List bytes, String remotePath) async {
    try {
      final b64File = '$remotePath.b64';

      // Encode to base64 then split into 64-char lines (standard PEM format).
      // openssl enc -base64 -d without -A expects exactly this format.
      final b64Raw = base64Encode(bytes);
      final b64Lines = <String>[];
      for (var i = 0; i < b64Raw.length; i += 64) {
        b64Lines.add(b64Raw.substring(i, (i + 64).clamp(0, b64Raw.length)));
      }

      // Write lines to .b64 file — batch 20 lines per SSH command to reduce
      // round-trips. Base64 chars [A-Za-z0-9+/=] are all shell-safe.
      const batchSize = 20;
      for (var i = 0; i < b64Lines.length; i += batchSize) {
        final batch = b64Lines.sublist(i, (i + batchSize).clamp(0, b64Lines.length));
        final cmds  = batch.asMap().entries.map((e) {
          final lineOp = (i == 0 && e.key == 0) ? '>' : '>>';
          return 'printf "%s\n" "${e.value}" $lineOp "$b64File"';
        }).join(' && ');
        final result = await run(cmds);
        if (result.trim().isNotEmpty) {
          await run('rm -f "$b64File"');
          return 'Write error at batch ${i ~/ batchSize}: $result';
        }
      }

      // Decode PEM base64 to binary. No -A needed — standard line-wrapped
      // base64 is openssl's native input format.
      final decodeResult = await run(
        'openssl enc -base64 -d -in "$b64File" -out "$remotePath" && rm -f "$b64File"');
      if (decodeResult.trim().isNotEmpty) {
        await run('rm -f "$b64File" "$remotePath"');
