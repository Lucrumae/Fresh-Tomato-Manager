...
class SshService {
  ...
  Future<Map<String, dynamic>> getTrafficHistory() async {
    try {
      // Signal rstats to flush, then read traff- nvram keys + /proc/net/dev
      // Shell loop using semicolons only (no backslash line continuation = no escaping issues)
      final raw = await run(
        ...
        '  [ -n "\$D" ] && V=\$(nvram get "traff-\$D" 2>/dev/null); '
        '  [ -n "\$D" ] && [ -n "\$V" ] && echo "\$D:\$V"; '
        'done 2>/dev/null; '
        'echo "=DEV="; '
        'cat /proc/net/dev'
