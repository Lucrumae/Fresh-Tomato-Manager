...
class RouterStatusNotifier extends StateNotifier<RouterStatus> {
  ...
  void startPolling() {
    ...
    fetchFull();
    _fast = Timer.periodic(const Duration(seconds:1),  (_) => fetchFast());  // CPU pool: 1s
...
class BandwidthNotifier extends StateNotifier<BandwidthStats> {
  ...
  void startPolling() { _timer?.cancel(); _poll(); _timer = Timer.periodic(const Duration(seconds:2), (_) => _poll()); }  // Bandwidth pool: 2s
  void stopPolling() => _timer?.cancel();
...
class LogsNotifier extends StateNotifier<List<LogEntry>> {
  ...
  void startPolling() { _timer?.cancel(); fetch(); _timer = Timer.periodic(const Duration(seconds:5), (_) => fetch()); }  // Log pool: 5s
  void stopPolling() => _timer?.cancel();
  Future<void> fetch() async {
    final ssh = _ref.read(sshServiceProvider);
    if (!ssh.isConnected) return;
