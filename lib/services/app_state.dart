...
class AccentNotifier extends StateNotifier<AccentColor> {
  AccentNotifier() : super(AccentColor.orange) { _load(); }
...
class RouterStatusNotifier extends StateNotifier<RouterStatus> {
  ...
  bool _inFlight = false; // guard: skip tick if previous still running
  ...
  void startPolling() {
    ...
    _inFlight = false;
    fetchFull();
    // Combined fast poll: CPU+RAM+temp+bandwidth in ONE SSH command (1s interval)
    _fast = Timer.periodic(const Duration(seconds:1), (_) => _fetchCombined());
    _slow = Timer.periodic(const Duration(seconds:15), (_) => fetchFull());
  }
  void stopPolling() { _fast?.cancel(); _slow?.cancel(); _inFlight = false; }
  void pausePolling() { _fast?.cancel(); _slow?.cancel(); }
  void resumePolling() { startPolling(); }

  Future<void> _fetchCombined() async {
    if (_inFlight) return; // previous command still running — skip this tick
    _inFlight = true;
    try {
      final ssh = _ref.read(sshServiceProvider);
      if (!ssh.isConnected) return;
      final result = await ssh.getCombinedFast(state);
      if (!mounted) return;
      state = result.status;
      // Forward bandwidth data to BandwidthNotifier
      _ref.read(bandwidthProvider.notifier).onRawSample(result.bandwidth);
    } finally {
      _inFlight = false;
    }
  }
...
class DevicesNotifier extends StateNotifier<List<ConnectedDevice>> {
  ...
  Future<void> fetch() async {
    ...
    // (new device detection retained for future use)
    if (_known.isNotEmpty && newMacs.isNotEmpty) {
      // new devices arrived — could log or alert here
    }
    _known = cur;
...
class BandwidthNotifier extends StateNotifier<BandwidthStats> {
  ...
  Map<String,int> _last = {'rx':0,'tx':0};
  DateTime? _lastTime;                         // actual timestamp of last sample
  double _totalRxMB = 0, _totalTxMB = 0;
  BandwidthNotifier(this._ref) : super(BandwidthStats.empty());
  // startPolling/stopPolling are no-ops — bandwidth is now driven by
  // RouterStatusNotifier._fetchCombined() via onRawSample() below.
  void startPolling() {}
  void stopPolling() {}

  // Called by RouterStatusNotifier every time a combined fast poll completes.
  void onRawSample(Map<String, int> s) {
    final now = DateTime.now();
    final rx  = s['rx'] ?? 0;
    final tx  = s['tx'] ?? 0;
    if (_lastTime == null) {
      _last = s; _lastTime = now; return;
    }
    final elapsed = now.difference(_lastTime!).inMilliseconds / 1000.0;
    if (elapsed < 0.5) return;

    // Guard counter wrap (uint32 overflow ~4GB) and router reboot
    // If rx/tx went backwards, reset baseline and skip this sample
    if (rx < _last['rx']! || tx < _last['tx']!) {
      _last = s; _lastTime = now; return;
    }

    final rd = rx - _last['rx']!;
    final td = tx - _last['tx']!;
    final rk = (rd / elapsed) / 1024 * 8;
    final tk = (td / elapsed) / 1024 * 8;

    _totalRxMB += rd / 1024 / 1024;
    _totalTxMB += td / 1024 / 1024;
    _last = s; _lastTime = now;
    _history.add(BandwidthPoint(time: now, rxKbps: rk, txKbps: tk));
...
class LogsNotifier extends StateNotifier<List<LogEntry>> {
  ...
  Future<void> fetch() async {
    ...
    for (final e in all) {
      // Key = time+message so identical messages at different timestamps all appear
      final key = '${e.time.millisecondsSinceEpoch}:${e.message}';
