...
// ── Config provider ────────────────────────────────────────────────────────────
final configProvider = StateNotifierProvider<ConfigNotifier, RouterConfig?>((ref) {
  return ConfigNotifier();
});

class ConfigNotifier extends StateNotifier<RouterConfig?> {
  ...
  Future<void> _load() async {
    ...
    if (json != null) {
      state = RouterConfig.fromJson(jsonDecode(json));
    }
  }

  Future<void> save(RouterConfig config) async {
    state = config;
    final prefs = await SharedPreferences.getInstance();
