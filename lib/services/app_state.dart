...
// Second SSH session — used exclusively for:
// bandwidth realtime, CPU/RAM polling, terminal, logs
// Shares the same router TCP connection but runs a separate shell channel.
final monitorSshServiceProvider = Provider<SshService>((ref) => SshService());
...
class RouterStatusNotifier extends StateNotifier<RouterStatus> {
  ...
  Future<void> fetchFast() async {
    // Monitor session: CPU/RAM fast poll (1s interval)
    final ssh = _ref.read(monitorSshServiceProvider);
...
class BandwidthNotifier extends StateNotifier<BandwidthStats> {
  ...
  void startPolling() { _timer?.cancel(); _poll(); _timer = Timer.periodic(const Duration(seconds:2), (_) => _poll()); }
  void stopPolling() => _timer?.cancel();
  Future<void> _poll() async {
    // Monitor session: realtime bandwidth (2s interval)
    final ssh = _ref.read(monitorSshServiceProvider);
    if (!ssh.isConnected) return;
    ...
    final tx  = s['tx'] ?? 0;
    if (_lastTime == null) {
      // First sample — just record baseline, no rate yet
      _last = s; _lastTime = now; return;
    }
    // Actual elapsed seconds since last sample (accounts for SSH command latency)
    final elapsed = now.difference(_lastTime!).inMilliseconds / 1000.0;
    if (elapsed < 0.5) return; // guard against timer drift / double-poll
    final rd = (rx - _last['rx']!).clamp(0, 999999999);
    final td = (tx - _last['tx']!).clamp(0, 999999999);
    // kbps = bytes_per_second * 8 / 1024, bytes_per_second = delta / elapsed
