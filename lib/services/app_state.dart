...
class AccentNotifier extends StateNotifier<AccentColor> {
  ...
  Future<void> set(AccentColor a) async {
    state = a;
    final p = await SharedPreferences.getInstance();
    await p.setInt('accent_color', AccentColor.values.indexOf(a));
  }
}
...
class ConfigNotifier extends StateNotifier<TomatoConfig?> {
  ...
  Future<void> save(TomatoConfig c) async {
    state = c;
    final p = await SharedPreferences.getInstance();
    await p.setString('router_config', jsonEncode(c.toJson()));
  }
  Future<void> clear() async {
    state = null;
    final p = await SharedPreferences.getInstance();
    await p.remove('router_config');
  }
}
...
class RouterStatusNotifier extends StateNotifier<RouterStatus> {
  ...
  void startPolling() {
    ...
    fetchFull();
    _fast = Timer.periodic(const Duration(seconds:1),  (_) => fetchFast());
    _slow = Timer.periodic(const Duration(seconds:5),  (_) => fetchFull());
  }
...
class DevicesNotifier extends StateNotifier<List<ConnectedDevice>> {
  final Ref _ref; Timer? _timer; Set<String> _known = {};
  ...
  Future<void> toggleBlock(String mac) async {
    ...
    final ok = await ssh.blockDevice(mac, !dev.isBlocked);
    if (ok && mounted) state = state.map((d) => d.mac == mac ? d.copyWith(isBlocked:!d.isBlocked) : d).toList();
  }
...
class PortForwardNotifier extends StateNotifier<List<PortForwardRule>> {
  ...
  void toggleRule(String id) {
    ...
    state = state.map((r) => r.id == id ? PortForwardRule(
      ...
      internalIp:r.internalIp, enabled:!r.enabled) : r).toList();
  }
  @override void dispose() { _timer?.cancel(); super.dispose(); }
}
