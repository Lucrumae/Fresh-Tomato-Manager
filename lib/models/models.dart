...
class ConnectedDevice {
  ...
  final bool isWan; // true = WAN gateway entry
  ...
  String get connectionType => isWan ? 'WAN' : (isWireless ? 'WiFi' : 'Ethernet');
...
class PortForwardRule {
  ...
  String name;
  String protocol;
  String externalPort; // can be "80", "1000:2000" (range), "80,443" (multi)
  String internalPort; // blank = same as external
  String internalIp;
  String srcFilter;    // source IP filter (blank = any)
  bool enabled;

  PortForwardRule({
    required this.id, required this.name, required this.protocol,
    required this.externalPort, required this.internalPort,
    required this.internalIp, this.srcFilter = '', required this.enabled,
  });
}
...
class LogEntry {
  ...
  final String rawLine;  // original line — used to detect new entries on poll
  ...
  bool get isSyslog  => source == 'system' && !isError && !isWarning;
}
